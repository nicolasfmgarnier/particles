...
public class PlayerBoard {
  ...
  public PlayerBoard(final int originX, final int originY, final int width, final int height, final int playerID, final SpriteBatch spriteBatch, final JaipurModel model, final Texture cardsTexture, final Texture tokensTexture, final Texture bonusTokensTexture, final Texture camelsActionsButtonsTexture, final Texture starTexture, final BitmapFont jaipurFont) {
  ...
  public void render() {
    ...
    if (model.playerCamelsSelected && model.playerTurn == playerID) {
  ...
  public void computeBoxes() {
    computeCardsBoxes();
    if (model.playerCamelsSelected && model.playerTurn == playerID) computeCamelsControlsBoxes();
    computeScoreBoxes();
  }
  ...
  private void computeCamelsControlsBoxes() {
    ...
    if (playerID == 0) {
      // Bottom player
      yOrigin = playerCamels.get(0).originY + playerCamels.get(0).height;
    } else {
      // Upper player
      yOrigin = playerCamels.get(0).originY - nbCamelsHeight;
    }
